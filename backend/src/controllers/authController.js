...
export const register = asyncHandler(async (req, res) => {
  ...
  await recordAuditEvent({ actor: user._id, action: 'auth.register', target: user.email, ip: req.ip });
  sendResponse(res, 201, { data: user.toSafeJSON(), message: 'Account created' });
});
